...
export class Entry extends ObjectBaseClass {
    ...
    axiosSuccess(response){
        switch(response.config.method.toUpperCase()){
            ...
            case "POST":
                return {type: 'success', message: "New entry created"};
            case "PUT":
                return {type: 'success', message: "Entry updated"};
            case "DELETE":
                return {deleted: true, notification: {type: 'success', message: "Entry was deleted"}}
        }
    }

    axiosSuccessTransfer(response){
        return {type: 'success', message: "Transfer entry created"};
    }
            ...
            switch(error.response.config.method.toUpperCase()){
                case "GET":
                    switch(error.response.status){
                        case 404:
                            return {fetched: false, notification: {type: 'warn', message: "Entry does not exist"}};
                        case 500:
                        default:
                            return {fetched: false, notification: {type: 'error', message: "An error occurred while attempting to retrieve entry"}};
                    }
                case "POST":
                    switch(error.response.status){
                        case 400:
                            return {type: 'warn', message: error.response.data.error};
                        case 500:
                        default:
                            return {type: 'error', message: "An error occurred while attempting to create an entry"};
                    }
                case "PUT":
                    switch(error.response.status){
                        case 400:
                        ...
                        case 500:
                        default:
                            return {type: 'error', message: "An error occurred while attempting to update entry [%s]"};
                    }
                case "DELETE":
                    switch(error.response.status){
                        case 404:
                            return {deleted: false, notification: {type: 'warn', message: "Entry [%s] does not exist and cannot be deleted"}};
                        case 500:
                        default:
                            return {deleted: false, notification: {type: 'error', message: "An error occurred while attempting to delete entry [%s]"}};
                    }
            }
        }
    }
