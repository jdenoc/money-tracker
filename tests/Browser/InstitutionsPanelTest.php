...
class InstitutionsPanelTest extends DuskTestCase {
    ...
    private function assertInstitutionPanelAccountNode(Browser $account_node, $account_types_collection, bool $has_tooltip=true){
        ...
        if($has_tooltip){
            // get the tooltip element id
            $account_node_tooltip_id = $account_node->attribute('', 'aria-describedby');
            $account_node_tooltip_selector = 'body #'.$account_node_tooltip_id;
            ...
            $account_node->assertVisible($account_node_tooltip_selector);
            $account_types_tooltip_text = $account_node->text($account_node_tooltip_selector);
            $account_node->resolver->prefix = $account_css_prefix;

            // account-type text is located in the tooltip
            ...
            $account_css_prefix = $account_node->resolver->prefix;
            $account_node->resolver->prefix = '';
            $account_node->assertMissing('body .v-popper__popper');
            $account_node->resolver->prefix = $account_css_prefix;
        }
