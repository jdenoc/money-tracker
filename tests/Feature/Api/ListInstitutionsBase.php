...
class ListInstitutionsBase extends TestCase {

    use DatabaseMigrations;

    protected $_base_uri = '';

    public function getInstitutionsWhenNoneAreAvailableTest(){
     ...
     */
    public function getInstitutionsTest($all_active){
        $faker = Factory::create();
        // GIVEN
        $institutions_count = $faker->randomDigitNotNull;
        $generated_institutions = [];
        for($i=0; $i<$institutions_count; $i++){
            $active_flag = $faker->boolean(($all_active? 100:50));
